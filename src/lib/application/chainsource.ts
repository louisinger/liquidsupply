...
function toAssetChainTransaction(a: EsploraTransaction, assetHash: string): AssetChainTransaction {
	const issuances = a.vin
		.filter((i) => i.issuance && i.issuance.asset_id === assetHash && i.issuance.assetamount)
		.map(({ issuance }) => {
			return {
...
export class EsploraChainSource implements ChainSource {
	...
	async getAsset(assetHash: string): Promise<Asset> {
		const response = await this.axiosInstance.get<EsploraAsset>(`/asset/${assetHash}`);
